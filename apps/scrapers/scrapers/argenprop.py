...
def build_url(zone: dict, operation: str, prop_type: str, page: int = 1) -> str:
    """Argenprop paginates via the curious `?pagina-N` format (hyphen, not equals).
    Verified by clicking 'Siguiente' on the live site — each click moves to
    `?pagina-2`, `?pagina-3`, ... and returns distinct items.

    robots.txt explicitly allows pages 1–10 (`Allow: /*?pagina-N$` for N in 1..10),
    so we cap at 10 by default to respect their crawl preference. ~200 items per
    (zone, op, type) combo.
    """
        ...
        *,
        ...
        try:
            for prop_type in types:
                ...
                # Argenprop's robots.txt only allows pages 1–10. Cap to honor that.
                for page_num in range(1, (max_pages or 10) + 1):
